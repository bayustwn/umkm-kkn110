...
export default function Navbar({ color }: NavbarProps) {
  ...
  return (
    <header className="flex justify-between items-center w-full relative z-50">
      <img
        src={`${color ? '/icons/logo-black.svg' : '/icons/logo.svg'}`}
        alt="logo"
        className="h-10"
      />

      <nav className="hidden md:flex flex-row gap-10 items-center text-medium">{navItems}</nav>
      ...
      <div
        className={`fixed inset-0 bg-black bg-black/90 flex flex-col items-center justify-center gap-10 text-xl text-white transform transition-transform duration-300 ease-in-out md:hidden ${
