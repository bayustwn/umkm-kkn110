...
export default function Sidebar() {
  ...
  return (
    <>
                ...
                <div className="space-y-3">
                  <div>
                    ...
                    <input
                      ...
                      onChange={(e) =>
                        setUserData((prev) => ({ ...prev, currentPassword: e.target.value }))
                      }
                      className="w-full px-3 py-2 border border-gray-300 rounded-lg focus:outline-none focus:ring-2 focus:ring-primary focus:border-transparent"
      ...
      <div
        className={`bg-white shadow-lg h-full fixed left-0 top-0 z-50 transition-transform duration-300 ease-in-out ${isOpen ? 'translate-x-0' : '-translate-x-full'} md:translate-x-0 w-64 flex-shrink-0`}
      >
          ...
          <ul className="space-y-2 px-4">
            {menuItems.map((item) => (
              <li key={item.path}>
                <button
                  onClick={() =>
                    item.isSettings ? setShowSettingsModal(true) : handleMenuClick(item.path)
                  }
                  className={`w-full text-sm cursor-pointer flex items-center px-4 py-3 rounded-lg transition-all duration-200 ${location.pathname === item.path ? 'bg-primary/10 text-primary' : 'text-gray-700 hover:bg-gray-100'}`}
                >
