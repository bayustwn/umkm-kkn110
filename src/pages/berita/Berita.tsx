  ...
  const renderNewsList = (items: typeof news) => (
    <div className="w-full gap-5 mt-10 grid grid-cols-1 md:grid-cols-2">
      {items.length === 0 ? (
        ...
        items.map((item) => (
          <NewsCard key={item.id} id={item.id} title={item.title} content={item.content} image={item.image} createdAt={item.created_at} onNavigate={goToBeritaDetail} />
        ))
      )}
    </div>
  );
        ...
        isLoading ? <FeaturedNewsSkeleton /> : (
          <>
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 gap-5 mt-8">
              {[0, 1].map((i) => news[i] && (
                <div key={news[i].id} onClick={() => goToBeritaDetail(news[i].id)} className="relative cursor-pointer h-90 md:h-full">
