      ...
      {showSuccessModal && (
        <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/40">
          <div className="bg-white rounded-lg flex flex-col items-center px-8 py-10 shadow-lg max-w-[90vw]">
            ...
            <button className="mt-2 px-8 py-2 rounded-full bg-primary text-white font-semibold text-lg hover:bg-primary/90 transition" onClick={() => goToUmkm()}>Lihat UMKM</button>
          </div>
        </div>
      )}
            ...
            <form onSubmit={handleSubmit}>
              {step === 1 && (
                <div className="flex flex-col gap-4">
                  ...
                  <label className="block"><span className="font-semibold text-lg">Kategori</span>
              ...
              {step === 2 && (
                <div className="flex flex-col gap-4">
                  <label className="block flex flex-col gap-1"><span className="font-semibold text-lg">Alamat UMKM</span><input placeholder="Alamat bisnis anda" className="border px-4 py-2 rounded-md focus:border-primary mt-1" value={umkmData.alamat} onChange={e => setUmkmData(d => ({ ...d, alamat: e.target.value }))} required /><p>Contoh. Jl Bibis Tama No. 10</p></label>
              ...
              {umkmPreview.slice(0, 2).map((umkm, index) => (
                <div key={index} className="flex flex-col gap-2 mb-3 md:mb-0 bg-white rounded-md">
                  <div className="relative">
                    ...
                    <div className="absolute top-3 left-3 justify-center bg-primary w-fit px-4 text-white border border-primary text-sm py-1 rounded-full"><p>{umkm.category}</p></div>
                  </div>
