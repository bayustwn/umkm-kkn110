    ...
    <div key={item.id} className="w-full flex flex-row gap-5 items-center min-w-0">
      ...
      <div className="flex flex-col gap-2 min-w-0 flex-1">
        ...
        <div className="flex gap-2 mt-2">
          ...
          <button onClick={() => goToNewsDetail(item.id)} className="text-sm font-medium text-primary hover:text-primary/80 transition-colors cursor-pointer">Lihat →</button>
        </div>
      </div>
    </div>
  );
    ...
    <div className="md:py-[2%] md:px-[2%]">
      <div className="flex flex-col md:flex-row justify-between items-center">
        ...
        <div className="flex flex-row items-center mt-5 md:mt-0 gap-2">
          ...
          <button onClick={() => setShowDateModal(true)} className="p-3 md:hidden rounded-full bg-primary cursor-pointer">
            <img src="/icons/date.svg" className="w-4" alt="date" />
          </button>
          <button className="bg-primary hidden md:flex px-5 cursor-pointer flex-row gap-2 hover:bg-primary/80 py-2 rounded-full text-white font-semibold" onClick={() => setShowDateModal(true)}>
            <img src="/icons/date.svg" className="w-5" alt="date" />
            <p>Filter Tanggal</p>
          </button>
          <button onClick={() => navigate('/admin/berita/tambah')} className="bg-primary hover:bg-primary/80 text-white p-2 rounded-full transition-colors cursor-pointer">
      ...
      <div className="mt-8">
        {filterActive && (
          <div className="mb-6 flex gap-5 items-center">
            ...
            <button className="px-4 py-2 rounded-4xl bg-primary text-white hover:bg-primary/80 cursor-pointer" onClick={() => { setFilterActive(false); setDateStart(''); setDateEnd(''); }}>Reset Filter</button>
          </div>
        )}
      ...
      {!filterActive && !search && news.length > page * perPage && (
        <div className="w-fit flex flex-row w-full mt-8 justify-center">
          <button className="border cursor-pointer hover:scale-105 transition-all py-2 px-5 rounded-full" onClick={() => setPage(page + 1)}>
      ...
      <DateFilterModal
        ...
        onApply={() => { setShowDateModal(false); setFilterActive(true); }}
