...
export default function UmkmAdmin() {
  ...
  const renderUmkmCard = (item: AdminUmkmItem) => (
    <div key={item.id} className="mt-5 flex w-full flex-col gap-2 mb-3 md:mb-0 bg-white rounded-md">
      ...
      <div
        className={`w-fit px-3 text-white border text-xs py-1 rounded-full mb-2 ${item.status === 'active' ? 'bg-green-500 border-green-500' : 'bg-yellow-500 border-yellow-500'}`}
      >
      ...
      <div>
        <div className="my-4">
          ...
          <div className="flex flex-row gap-2 mt-3 items-center">
            ...
            <p className="text-sm">{item.productCount} Produk</p>
          </div>
        </div>
        <div className="mt-5 flex flex-row items-center justify-between">
          <div className="flex flex-col items-start">
            <p className="text-sm">Mulai dari</p>
            <p className="text-lg font-bold">Rp {item.lowestPrice}</p>
          </div>
          <div className="flex gap-2 mt-2">
            <button
              onClick={() => setUmkmToDelete(item)}
              className="text-sm font-medium text-red-600 transition-colors cursor-pointer"
            >
              Hapus
            </button>
            <button
              onClick={() => navigate(`/admin/umkm/edit/${item.id}`)}
            ...
            <button
              onClick={() => goToUmkmDetail(item.id)}
  ...
  return (
    <div className="py-10 md:px-0 md:py-[2%] md:px-5">
          ...
          {isLoading ? (
            ...
            <div className="grid grid-cols-1 md:grid-cols-3 gap-5">
              {filteredUMKM.length === 0 ? (
                ...
                filteredUMKM.map(renderUmkmCard)
              )}
            </div>
          )}
        </div>
      ...
      <ConfirmModal
        isOpen={!!umkmToDelete}
        onClose={() => setUmkmToDelete(null)}
        onConfirm={() => {
          if (!umkmToDelete) return;
          deleteUmkm.mutate(umkmToDelete.id, { onSettled: () => setUmkmToDelete(null) });
        }}
        title="Konfirmasi Hapus UMKM"
        message={`Apakah Anda yakin ingin menghapus UMKM <span class="font-semibold">"${umkmToDelete?.name}"</span>?<br/><span class="text-red-600 text-xs">Semua produk, foto UMKM, dan foto produk yang terkait juga akan dihapus.</span>`}
        confirmText="Hapus"
        isLoading={deleteUmkm.isPending}
        variant="danger"
      />

      <ConfirmModal
        isOpen={!!umkmToApprove}
        onClose={() => setUmkmToApprove(null)}
        onConfirm={() => {
          if (!umkmToApprove) return;
          approveUmkm.mutate(umkmToApprove.id, { onSettled: () => setUmkmToApprove(null) });
        }}
        title="Konfirmasi Setujui UMKM"
        message={`Apakah Anda yakin ingin menyetujui UMKM <span class="font-semibold">"${umkmToApprove?.name}"</span>?`}
        confirmText="Setujui"
        isLoading={approveUmkm.isPending}
        variant="success"
      />
      ...
      <ConfirmModal
        ...
        onConfirm={() => {
          ...
          setSelectedCategories((prev) => prev.filter((c) => c !== categoryToDelete.name));
        }}
        title="Konfirmasi Hapus Kategori"
        message={`Apakah Anda yakin ingin menghapus kategori <span class="font-semibold">"${categoryToDelete?.name}"</span>?<br/><span class="text-red-600 text-xs">Kategori yang masih digunakan oleh UMKM tidak dapat dihapus.</span>`}
        confirmText="Hapus"
          ...
          <div className="relative bg-white rounded-lg shadow-xl p-6 w-full max-w-md mx-4">
            <div className="text-center">
              ...
              <div className="flex gap-2 justify-center">
                ...
                >
                  {createCategory.isPending ? 'Menambahkan...' : 'Tambah Kategori'}
                </button>
              </div>
            </div>
          </div>
